...
const username = "mouse0910@gmail.com";
const password = "Closable-Gong9-Duller";
...
class Komga implements Plugin.PluginBase {
    ...
    async popularNovels(
        ...
        const body = await fetch(link + new URLSearchParams({
        ...
        return body.content.map((book: { name: string; id: string; }): Plugin.NovelItem => ({
            name: book.name,
            path: `/books/${book.id}`,
    ...
    async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
        ...
        const bookBody = await fetch(this.site + novelPath, { headers: this.headers }).then((r) => r.json());
        const seriesBody = await fetch(this.site + `/series/${bookBody.seriesId}`, { headers: this.headers }).then((r) => r.json());
        ...
        // novel.artist = "";
        novel.author = bookBody.metadata.authors.map((author: { name: string; }) => author.name).join(", ");
        console.log("Bruh")
        ...
        const chaptersBody = await fetch(this.site + `/books/${bookBody.id}/manifest/epub`, { headers: this.headers }).then((r) => r.json());

        console.log(chaptersBody)

        let chapters = chaptersBody.readingOrder.map((chapter: { href: string; }, index: number): Plugin.ChapterItem => {

            const matchingTocEntry = chaptersBody.toc.find((tocEntry: { href: string; title: string; }) => tocEntry.href === chapter.href);
            return {
    ...
    async parseChapter(chapterPath: string): Promise<string> {
        const body = await fetchApi(chapterPath, { headers: this.headers }).then((r) => r.text());

        console.log("Parsing chapter", chapterPath)

        const $ = loadCheerio(body);

        const baseUrl = chapterPath.substring(0, chapterPath.indexOf("/OEBPS/") + "/OEBPS/".length);
